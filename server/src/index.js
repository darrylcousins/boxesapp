...
if (!isTest) {
  createServer().then(({ app, vite }) => {
    ...
    io.on('connection', function(socket) {
      ...
      socket.on("connectInit", async sessionId => {

        console.log('connectInit', socket.id, sessionId);
        ...
        // e.g req.app.sockets[sessionId]
        socket.emit("connected", sessionId); // feedback to client
      })
    ...
    /* E.G. in a request
        ...
        const thisSocketId = req.app.get('sockets')[sessionId];
